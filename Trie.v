...
try (rewrite IHp; omega); reflexivity.
Qed.
...
Proof. intros c p. revert c.  
  induction p; 
  intros; destruct c, q;
  simpl; try rewrite IHp; 
  try rewrite succ_correct; omega.
Qed.
...
Proof.
  ...
  destruct (compare x y) eqn:H; omega);
  try (pose (positive2nat_pos x); omega);
  try (pose (positive2nat_pos y); omega);
  reflexivity.
Qed.
