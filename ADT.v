 ...
 Theorem gso: forall j k v t,  j<>k -> get j (set k v t) = get j t.
   Proof. intros. unfold get, set.
     destruct (unrealistically_strong_can_relate V default t)
        as [cts H'].
     assert (H0 := insert_relate V default k v t cts H').
     assert (H1 := lookup_relate V default j _ _ H0).
     assert (H2 := lookup_relate V default j _ _ H').
     rewrite H1, H2. auto using t_update_neq.
Qed.
...
(** Prove this using techniques similar to the proof of [gss] just above;
 ...
 Theorem gso: forall j k v t,  j<>k -> get j (set k v t) = get j t.
   Proof. intros. unfold get, set.
     destruct t as [t HH]. simpl.
     destruct (can_relate V default t HH)
        as [cts H'].
     assert (H0 := insert_relate V default k v t cts H').
     assert (H1 := lookup_relate V default j _ _ H0).
     assert (H2 := lookup_relate V default j _ _ H').
...
| L_Abs_nil : forall a b c l, L_Abs (a, b, c) (a :: b :: c :: l).

Definition O_Abs al al' := L_Abs al al'.

(* State these theorems using O_Abs, not L_Abs.
   You'll see why below, at "Opaque". *)
Lemma create_relate : forall a b c, O_Abs (L.create a b c) [a; b; c].
Proof. intros; constructor. Qed.

Lemma cons_relate : forall al al' i, 
  O_Abs al al' -> O_Abs (L.cons i al) (i :: al').
Proof. induction 1. constructor. Qed.

Lemma nth_relate : forall al al' i, 
  O_Abs al al' -> L.nth i al = nth i (firstn 3 al') 0.
Proof. induction 1.  unfold L.nth, firstn, nth. 
repeat (destruct i; auto). Qed.
...
Proof. 
  induction 1; intros.
  unfold step. simpl. 
  replace (a, b, c) with (L.create a b c); auto.
  repeat (erewrite nth_relate; 
        [|apply create_relate]).
  constructor.
Qed.
...
Proof. induction 1. induction n.
  simpl. constructor. simpl.
  apply step_relate. assumption.
Qed.


Lemma fibish_correct: forall n, fibish n = fib n.
Proof. unfold fibish, fib. intro.
  rewrite nth_relate with (al':=repeat step [1; 0; 0] n).
  apply nth_firstn; omega.
  apply repeat_step_relate. 
  apply create_relate.
Qed.
